...
class ABSAProcessor(DataProcessor):
    ...
    def _create_examples(self, data_dir, set_type, tagging_schema):
        ...
        with open(file, 'r', encoding='utf-8') as fp:  # JULIAN From UTF-8 to cp1252
            sample_id = 0
            for line in fp:
                line = line.strip()
                if not line:
                ...
                try:
                    sent_string, tag_string = line.split('####')
                except ValueError:
                    print(sample_id, data_dir, set_type, f'[{line}]')
                sent_string, tag_string = line.split('####')
                # if not tag_string:  # JULIAN added
                #     continue
                words = []
                tags = []
...
processors = {
    ...
    "commonsents": ABSAProcessor,
    "newsprepared": ABSAProcessor,
}
...
output_modes = {
    ...
    "commonsents": "classification",
    "newsprepared": "classification",
}
