...
print("cuda available inside:", torch.cuda.is_available())
...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.max_steps > 0:
        ...
        args.num_train_epochs = args.max_steps // (len(train_dataloader) // args.gradient_accumulation_steps) + 1  # JULIAN ADDED 6
    else:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            #  JULIAN make sure loss isn't including non masked terms. 
...
def main():
    ...
    print("localrank:", args.local_rank, "nocuda:", args.no_cuda)
    if args.local_rank == -1 or args.no_cuda:
        print("cuda is avail:", torch.cuda.is_available(), "args_no_cuda:", args.no_cuda)
        device = torch.device("cuda" if torch.cuda.is_available() and not args.no_cuda else "cpu")
        args.n_gpu = torch.cuda.device_count()
    else:  # Initializes the distributed backend which will take care of sychronizing nodes/GPUs
        torch.cuda.set_device(args.local_rank)
        print("set device", args.local_rank)
