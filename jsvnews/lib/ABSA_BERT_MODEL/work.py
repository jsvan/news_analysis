...
def predict(args, model, tokenizer):
    ...
    for batch in tqdm(dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            for t in p_ts_sequence:
                beg, end, sentiment = t
                # JULIAN aspect = words[beg:end+1]
                # JULIAN output_ts.append('%s: %s' % (aspect, sentiment))
                output_ts.append(sentiment)
            print("Input: %s, output: %s" % (' '.join(words), '\t'.join(output_ts)))
            #tofile.append('$$$ '+ ' '.join(words))
            tofile.append('\t\t'.join(output_ts))
            """if (count+1) % 1501 == 0:
                print("Writing to disk")
                with open(os.path.join(args.data_dir, "outputs", f"labels_{count}.txt"), 'w', encoding='utf-8') as F:
                    F.write('\n'.join(tofile))
                tofile.clear()
            """
            count += 1
    ...
    with open(os.path.join(args.data_dir, "outputs", "labels.txt"), 'w', encoding='utf-8') as F:
        F.write('\n'.join(tofile))
